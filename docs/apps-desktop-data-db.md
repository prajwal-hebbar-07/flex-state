...
3. `migrate()` separately adds `video` to legacy exercise tables and `body_focuses` with a JSON `[]` default to legacy personalization tables, swallowing duplicate-column errors on current installs.
...
8. The migration then rebuilds `personalization` into its current shape, writing `[]` to `body_focuses`, the new location id to `location_id`, and leaving `generator_version` at its old value so the next load returns `regeneration_required`.
...
14. Loading parses and validates `body_focuses` plus the remaining profile fields before checking generator version or plan JSON. It does not check the location id against the location list; that reconciliation belongs to `App`.
...
17. `savePersonalization()` serializes `body_focuses` and the plan, then replaces every saved field with one `INSERT ... ON CONFLICT(id) DO UPDATE` execution. It writes `location_id` and no longer writes `has_dumbbells` or `excluded_exercise_slugs`; neither column exists.
...
- The SQLite table constrains profile literals, booleans, days, duration, and fixed id; runtime validation constrains the JSON body-focus array.
